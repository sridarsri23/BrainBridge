...
from server.models import User, ProfileNDAdult
from server.schemas import ProfileNDAdultResponse, ProfileNDAdultCreate, ProfileNDAdultUpdate
...
async def get_profile(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    profile = db.query(ProfileNDAdult).filter(ProfileNDAdult.nd_adult_id == current_user.id).first()
    
    if not profile:
        # Create default profile if none exists
        profile = ProfileNDAdult(
            nd_adult_id=current_user.id,
...
async def create_or_update_profile(
    ...
    existing_profile = db.query(ProfileNDAdult).filter(ProfileNDAdult.nd_adult_id == current_user.id).first()
